...
def train_epoch(epoch, train_loader, model, criterion, optimizer, lr_scheduler, w_loss, report, max_epoch, writer):
    ...
    for i, (range_data, polar_data, r2p_matrix, p2r_matrix, knns) in enumerate(train_loader):
        ...
        d3 = time.time()
        loss_fusion = criterion(fusion_pred, unproj_labels) + lovasz_softmax(torch.nn.functional.softmax(fusion_pred, dim=1), unproj_labels, ignore=255)
        loss_range = criterion(range_pred, unproj_labels) + lovasz_softmax(torch.nn.functional.softmax(range_pred, dim=1), unproj_labels, ignore=255)
        loss_polar = criterion(polar_pred, unproj_labels) + lovasz_softmax(torch.nn.functional.softmax(polar_pred, dim=1), unproj_labels, ignore=255)

        # l1 = loss_bd(torch.nn.functional.softmax(range_x, dim=1), proj_labels.long())
