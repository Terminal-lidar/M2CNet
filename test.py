...
from models.M2CNet import M2CNet
...
def parse_args():
    parser = argparse.ArgumentParser(description='M2CNet for LiDAR Point Clouds Semantic Segmentation')
    parser.add_argument(
        '--dataset', '-d',
        type=str,
        required=True,
    ...
    with torch.no_grad():
        ...
        for i, (range_data, polar_data, r2p_matrix, p2r_matrix, knns) in enumerate(data_loader):
            ...
            for i, pred in enumerate(pred_to_test):
                ...
                name = path_name[i]
                seq = path_seq[i]
                path = os.path.join(str(log_path), 'sequences',
                                    str(seq), 'predictions', str(name))
